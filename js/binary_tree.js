class BinaryTree {
    // inicializa a raiz como nula                                                          //Criação do construtor
    constructor() {                                                                         //A raiz da Arvore recebe null
        this.root = null
    }
    ...
    removeVisitor(node, value) {                                                            //Recebe os parâmetros
        if (node.content == value) {                                                        //Verifica se o nó é igual ao valor
            if (node.left == node.right) {                                                  //Verifica se o nó da esquerda é igual ao nó da direita
                ...
                return node.right                                                           //não tem filhos da esquerda, e tem nó da direita - Grau 1
            } else {
    ...
    sizeVisitor(node) {                                                                     //Verifica se há algum nó
        if (!node)
